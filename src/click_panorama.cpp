...
//
//    //特徴点選択
//    for (auto itr = imList.begin(); itr != imList.end(); ++itr) {
//
//        //クリックで特徴点選択
//        //ハードルのマーカーをクリック
...
//
//    //特徴点選択
//    for (int i = 1; i < imList.size() - 1; i++) {
//
//
//        cv::Mat mask = maskAofB(imList[i].trackArea, imList[i].clickMask);
//
//        cv::imshow("gaga", imList[i].trackArea);
//        cv::waitKey(0);
//        cv::imshow("gaga", imList[i].clickMask);
//        cv::waitKey(0);
//
//        cv::Point2f pt1, pt2;
...
//        imList[i + 1].clickMask = insidelineMask;
//
...
//
//        cv::matchTemplate(imList[i].gray_image, featureArea, result_templete, CV_TM_CCOEFF);
//        cv::Point maxpt;
//        int max = 0;
...
////    cv::Mat panorama = imList[0].image;
////
////    for (int i = 0; i < imList.size() - 1; i++) {
////
////        //load images
////        cv::Mat to_image = imList[i].image;
////        cv::Mat from_image = imList[i + 1].image;
////
////        //load feature points
...
////
////            vector<cv::Point2f> to_points = imList[i].image;
////            vector<cv::Point2f> from_points = imList[i].image;
